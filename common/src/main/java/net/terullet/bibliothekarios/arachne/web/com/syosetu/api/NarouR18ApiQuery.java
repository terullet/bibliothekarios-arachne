...
import net.terullet.bibliothekarios.arachne.web.com.syosetu.*;

import java.net.URI;
import java.util.Collections;
import java.util.Set;

public final class NarouR18ApiQuery extends NarouApiQuery {
	private final Set<Item> items;
	private final Set<NarouR18Genre> includingGenres;
	private final Set<NarouR18Genre> excludingGenres;
	private final URI uri;

	public NarouR18ApiQuery(
			Set<Item> items,
			int limit,
			int from,
			...
			Set<NarouR18Genre> includingGenres,
			Set<NarouR18Genre> excludingGenres,
			int minCharacters,
			int maxCharacters,
		...
		StringBuilder sb = new StringBuilder("https://api.syosetu.com/novel18api/api/?gzip=5&out=json&of=n");
		for (Item i : this.items) {
			sb.append('-').append(i.getOfKey());
		}
		if (!this.includingGenres.isEmpty()) {
			sb.append("&nocgenre=");
			for (NarouR18Genre g : this.includingGenres) {
				sb.append(g.getId()).append('-');
			}
			sb.deleteCharAt(sb.length()-1);
		}
		if (!this.excludingGenres.isEmpty()) {
			sb.append("&notnocgenre=");
			for (NarouR18Genre g : this.excludingGenres) {
				sb.append(g.getId()).append('-');
			}
			sb.deleteCharAt(sb.length()-1);
		}
	...
	public Set<NarouR18Genre> getIncludingGenres() {
		return this.includingGenres;
	}
	public Set<NarouR18Genre> getExcludingGenres() {
