...
class NarouNcodeParser {
	...
	private InputStream decompressInputStream(HttpResponse<InputStream> httpResponse) throws IOException {
		return switch (HttpEncoding.fromKey(httpResponse.headers().firstValue("Content-Encoding").orElse(""))) {
	...
	private static final DateTimeFormatter NAROU_EPISODE_DATETIME_FORMAT = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm");
		...
		for (Element e : doc.select(".p-eplist").getFirst().children()) {
			switch (e.className()) {
				...
				case "p-eplist__sublist":
					...
					ZonedDateTime postedAt = ZonedDateTime.of(LocalDateTime.parse(postedAtMatcher.group("date") + " " + postedAtMatcher.group("time"), NAROU_EPISODE_DATETIME_FORMAT), ZoneOffset.ofHours(9));
					...
					if (!tues.isEmpty()) {
						...
						lastUpdatedAt = ZonedDateTime.of(LocalDateTime.parse(lastModifiedAtMatcher.group("date") + " " + lastModifiedAtMatcher.group("time"), NAROU_EPISODE_DATETIME_FORMAT), ZoneOffset.ofHours(9));
					}
					episodes.add(new NarouEpisodeResponseMetadata(response.work().ncode(), episodeNumber, titleElement.ownText(), postedAt, lastUpdatedAt));
					break;
				default:
